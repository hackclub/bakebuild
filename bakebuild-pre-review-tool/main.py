...
STEP_POOL = ProcessPoolExecutor(max_workers=1)
...
def safe_step_dimensions(filepath, timeout=10):
...
def main():
    ...
    for item in download_results:
        ...
        fits = dx is not None and dx <= LIMIT_MM and dy <= LIMIT_MM and dz <= LIMIT_MM
        ...
        final_rows.append({
            ...
            "Filename": filename,
            "Size X (mm)": f"{dx:.2f}" if dx else "",
            "Size Y (mm)": f"{dy:.2f}" if dy else "",
            "Size Z (mm)": f"{dz:.2f}" if dz else "",
            "FitsFlag": fits,
            "Fits 100mm³?": "YES" if fits else ("NO" if dx else "ERROR"),
